...
using internal::soilBlockedAt;
using internal::terrainHeightAt;
using internal::terrainSlopeAt;
            ...
            if (rootVigor > 1e-4f && sp.seedingRadius > 0.0f) {
                ...
                if (plant.age > fEff) {
                    ...
                    if (randFloat01(rng) < p) {
                        ...
                        // Drop onto terrain if available; reject when
                        // soil is blocked or the slope is too steep for
                        // the species. Slope is measured as the angle
                        // between the surface normal and +Y, so the
                        // default π/2 cap admits everything that isn't
                        // an overhang.
